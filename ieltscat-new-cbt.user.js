// ==UserScript==
// @name         雅思猫机考 · 新版雅思机考界面
// @name:en      IELTS Cat Mock → New CBT Interface
...
`;
  ...
  function renderNotesList() {
    ...
    ids.forEach(function (id) {
      ...
      ta.addEventListener('input', function () {
        var d = getNoteData();
        if (d[id]) { d[id].note = ta.value; LS.set(KEYS.noteData + location.pathname, JSON.stringify(d)); }
      });
      item.querySelector('.cbt-note-del').addEventListener('click', function () {
        /* 删除 localStorage 条目 */
        var d = getNoteData();
        delete d[id];
        LS.set(KEYS.noteData + location.pathname, JSON.stringify(d));
        /* 删除蓝色 mark（unwrap） */
  ...
  function getNoteData() {
    try { return JSON.parse(LS.get(KEYS.noteData + location.pathname, '{}')); }
    catch (e) { return {}; }
  }
  function saveNoteEntry(id, txt, note) {
    var d = getNoteData();
    d[id] = { text: txt, note: note };
    LS.set(KEYS.noteData + location.pathname, JSON.stringify(d));
  }
  ...
  function updatePartChecks() {
    ...
    $$('.exam-bank-footer .part').forEach(function (part) {
      ...
      var answered = 0;
      items.forEach(function (it) { if (it.classList.contains('had')) answered++; });
      var allDone = total > 0 && answered === total;
      var isCurrent = !!part.querySelector('.item.selected');
      var existing = title.querySelector('.cbt-check');
      if (allDone && !existing && isCurrent) {
   ...
   * ============================================================ */
  function revertAll() {
    /* 1. 把 clock-info 移回原始位置 */
    var clock = document.getElementById('clock-info');
    if (clock && clockOrigParent) {
    ...
    btns.forEach(function (b) {
      ...
      if (/submit/i.test(b.textContent || '')) {
        ...
        b.addEventListener('click', function () {
          ...
          /* 清空 localStorage 笔记数据 */
          try {
            Object.keys(localStorage).forEach(function (k) {
              if (k.indexOf('cbt-mock-note-data:') === 0 || k.indexOf('cbt-mock-notes:') === 0) {
  ...
  function boot() {
    ensureStyle();
    transformAll();
    /* 切题/切 Part 时（URL 变化）重跑 transformAll */
    ...
    requestAnimationFrame(checkUrl);
    /* 题号完成勾：用低频定时器更新，不用 MutationObserver 避免死循环 */
    setInterval(function () {
